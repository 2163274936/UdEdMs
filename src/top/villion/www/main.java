...
public class main {
    public static void main(String[] args) {
        ...
        String filename = "packages.csv";
        ArrayList<PackageInfo> packages = PackageIO.loadPackagesFromFile(filename);

        new GUI(packages);

        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            PackageIO.savePackagesToFile(packages, filename);
        }));
    }
}


//因改用从文件加载数据初始化集合，原创建空集合的方式已被替代而废弃。
...
//                    }
//                case "4":
//                    savePackagesToFile(packages,filename);
